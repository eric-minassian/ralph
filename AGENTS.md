...
- **`exactOptionalPropertyTypes` forbids assigning undefined to optional props**: With this tsconfig flag, `sms?: T` means the property can be omitted but cannot be explicitly set to `undefined`. Use flat boolean fields instead of optional nested objects to sidestep this. For objects built dynamically, use conditional assignment: `const obj: Foo = { required }; if (val !== undefined) { obj.optional = val; }`.
- **Use `TFunction` from i18next for validation function params**: When passing a translation function to standalone validation helpers, type the parameter as `TFunction` from `'i18next'` — not as a simplified `(key: string) => string`. TFunction is branded and simpler function types won't satisfy it.
- **i18next `count` must be a number**: When using `t('key', { count: ... })`, the `count` value must be `number`, not `string`. Using `String(n)` causes a type error with `TOptionsBase`.
- **Cognito `ClientSecretDescriptorType` uses prefixed field names**: The SDK type uses `ClientSecretId`, `ClientSecretValue`, `ClientSecretCreateDate` — not `SecretId`/`SecretValue`/`CreationDate`. The `AddUserPoolClientSecret` response wraps it in `ClientSecretDescriptor`, and `DeleteUserPoolClientSecret` requires `ClientSecretId` not `SecretId`.
